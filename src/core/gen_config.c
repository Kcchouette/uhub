 ...
 * Created 2026-01-03 01:50, by config.py
 */
...
void config_defaults(struct hub_config* config)
{
	...
	config->tls_cipher_list = hub_strdup("DEFAULT:@SECLEVEL=2:!ARIA:!CAMELLIA:!DSS:!MD5:!PSK:!SRP:!aNULL:!eNULL");
	config->tls_ciphersuites = hub_strdup("");
	...
	config->msg_hbri_timeout = hub_strdup("HBRI validation timeout");
	config->msg_hbri_validation_failed = hub_strdup("HBRI validation failed");
	config->msg_hbri_ip_mismatch = hub_strdup("IP address mismatch in HBRI validation");
}

int apply_config(struct hub_config* config, const char* key, const char* data, int line_count)
{
	int max = 0;
	int min = 0;
	...
	if (!strcmp(key, "server_port"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"server_port\" (integer), default=1511, min=1, max=65535");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "server_listen_backlog"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"server_listen_backlog\" (integer), default=50, min=5");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "failover_redirect_addr"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"failover_redirect_addr\" (string), default=\"\"");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "max_recv_buffer"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"max_recv_buffer\" (integer), default=4096, min=1024, max=1048576");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "max_send_buffer"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"max_send_buffer\" (integer), default=131072, min=2048");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "max_send_buffer_soft"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"max_send_buffer_soft\" (integer), default=98304, min=1024");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "limit_min_share"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"limit_min_share\" (integer), default=0");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "flood_ctl_interval"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"flood_ctl_interval\" (integer), default=0, max=60");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "tls_cipher_list") || !strcmp(key, "tls_ciphersuite"))
	{
		if (!apply_string(key, data, &config->tls_cipher_list, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"tls_cipher_list\" (string), default=\"DEFAULT:@SECLEVEL=2:!ARIA:!CAMELLIA:!DSS:!MD5:!PSK:!SRP:!aNULL:!eNULL\"");
			return -1;
		}
		return 0;
	}
	...
	{
		if (!apply_string(key, data, &config->tls_ciphersuites, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"tls_ciphersuites\" (string), default=\"\"");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "tls_version"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"tls_version\" (string), default=\"1.2\"");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "file_plugins"))
	{
		...
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"file_plugins\" (file), default=\"\"");
			return -1;
		}
		return 0;
	}
	...
	if (!strcmp(key, "msg_hbri_timeout"))
	{
		if (!apply_string(key, data, &config->msg_hbri_timeout, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"msg_hbri_timeout\" (message), default=\"HBRI validation timeout\"");
			return -1;
		}
		return 0;
	}

	if (!strcmp(key, "msg_hbri_validation_failed"))
	{
		if (!apply_string(key, data, &config->msg_hbri_validation_failed, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"msg_hbri_validation_failed\" (message), default=\"HBRI validation failed\"");
			return -1;
		}
		return 0;
	}

	if (!strcmp(key, "msg_hbri_ip_mismatch"))
	{
		if (!apply_string(key, data, &config->msg_hbri_ip_mismatch, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			LOG_ERROR("\"msg_hbri_ip_mismatch\" (message), default=\"IP address mismatch in HBRI validation\"");
			return -1;
		}
		return 0;
	}
